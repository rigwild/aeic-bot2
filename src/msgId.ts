...
export default {
  ...
  NOT_IN_CHANNEL: (channel: string) => `La commande ne peut être exécutée que dans le channel ${channel}. ${needHelp}`,
  MISSING_ROLE: (role: string) => `Le rôle ${role} est nécessaire pour exécuter cette commande. ${needHelp}`,
  UNKNOWN_GROUP: (tpGroup: string) => `Le groupe ${tpGroup} n'existe pas. ${needHelp}`,
